...
## SP7 — Gitea (mission app #2) acceptance gotchas

Harvested during the Task 12 lab acceptance run for **Gitea**, the first off-the-shelf
(non-cosmos) app onboarded through `srectl app install`
(`.superpowers/sdd/gitea-onboarding-plan/task-12a-report.md`,
`task-12a-rerun-report.md`, `task-12a-functional-report.md`). Where the T0–SP6 catalog
above surfaced platform gotchas during *initial bring-up*, these surfaced while installing
a **second app onto an already-running substrate** — a different failure surface (secret
provisioning, hook lifecycle, preflight tooling) than standing the platform up the first
time.

> **Numbering note:** this runbook has carried two schemes since SP1 — the early "RKE2/UDS
> gotcha cheat-sheet" (#1–7, mid-doc) and the later, superset "complete k3d → RKE2/UDS
> gotcha catalog" (#1–12) that subsumed it. New entries below continue the latter
> (canonical, highest-numbered) scheme from **#13**.

| # | Surprise | Symptom | Fix |
|---|---|---|---|
| 13 | DHCP can reassign the node's IP across a reboot; etcd's member list still pins the old one | `rke2-server` loops in `activating`: `this server is not a member of the etcd cluster. Found [...=https://192.168.86.51:2380], expect: ...=https://192.168.86.106:2380` | pin the node to a **static IP matching etcd's member list** — netplan config matched by MAC (`dhcp4: false`, `addresses: [192.168.86.51/24]`), disable cloud-init's network management (`echo "network: {config: disabled}" > /etc/cloud/cloud.cfg.d/99-disable-network-config.cfg`), `netplan apply`, then `systemctl restart rke2-server` |
| 14 | Falco's inotify watches exhaust the stock kernel limit as the pod count grows past what it was tuned for | Falco `CrashLoopBackOff` post-reboot: `could not initialize inotify handler` | `fs.inotify.max_user_instances` was still the kernel default `128` against ~70 pods on the node — raise it (`sysctl fs.inotify.max_user_instances=8192`, persist via `/etc/sysctl.d/99-inotify-falco.conf`), then delete the crashing pod |
| 15 | A namespace pre-created out-of-band (`kubectl create ns`, e.g. to seed SOPS secrets *before* an app's first install — a load-bearing step of the onboarding brief) never receives zarf's per-namespace `private-registry` image-pull secret | every pod in the namespace deadlocks `ImagePullBackOff`/`ErrImagePull`: `pull access denied ... no basic auth credentials`; kubelet event `FailedToRetrieveImagePullSecret (private-registry)` | zarf's `private-registry` secret provisioning appears to trigger off **zarf itself creating/owning the Namespace object** — pre-creating the namespace short-circuits that trigger (confirmed: 18/18 other, zarf-created namespaces on the lab have the secret; the out-of-band `gitea` namespace had 0/1). Fix: copy the `private-registry` `dockerconfigjson` secret from any already-provisioned namespace into the new one before its first `zarf`/`uds`-mediated deploy. Directly confirmed effective: images that failed with the auth error before pulled cleanly after the copy |
| 16 | A cluster's **first-ever** workload of a given image family pulling through the in-cluster registry can appear to self-heal from `ImagePullBackOff`/404 after several minutes with zero visible operator action | `crunchydata/crunchy-postgres`/`crunchy-pgbackrest` pods sat `ImagePullBackOff` → clean registry `404 not found` for ~14 minutes (the first non-`cosmos` `PostgresCluster` ever created on this lab), then went straight to `Running` | **Corrected finding — the first write-up of this was WRONG.** It was initially attributed to zarf/the in-cluster registry lazily mirroring images on first use ("propagation delay, self-resolving, no action needed"). It was not: the images were placed by a **manual, surgical registry seed** — an operator, working from a separately-permissioned session, copied/pushed the exact image the pod's kubelet event names into the in-cluster registry under its **exact crc-tag** (e.g. `127.0.0.1:31999/crunchydata/crunchy-postgres:ubi9-16.14-2621-zarf-2116639277`, `127.0.0.1:31999/crunchydata/crunchy-pgbackrest:ubi9-2.58.0-2621-zarf-2753312262`). The recipe: read the crc-tag **verbatim** off the pod's `ImagePullBackOff`/`Failed` event (`kubectl describe pod`), `crictl pull <upstream-ref>` (or equivalent), then retag/push into `127.0.0.1:31999/<repo>:<exact-crc-tag>` (e.g. via `ctr images tag` + `ctr images push`, or `skopeo copy`) — do not wait for it to "resolve on its own"; nothing in zarf actually does that |
| 17 | A single post-install hook Job's own failure can roll back an **entire, otherwise fully-healthy release** — including resources the failing Job doesn't even gate | the `gitea-minio-bootstrap` Job (see #18) hit its `activeDeadlineSeconds` (900s) after looping on an unrelated permission bug; the wrapper release's failure-handling then tore down the already-`4/4 Running` `PostgresCluster` and the already-`Initialized/green` MinIO `Tenant` right along with it — expensive-to-reprovision state destroyed by an unrelated script bug | no generic runbook fix — flagged as a **design consideration** for any chart with a post-install/post-upgrade hook Job: such a Job's failure should not be allowed to roll back core provisioning it doesn't functionally gate. Either give it a much longer deadline than the resources it might roll back need to provision, or decouple its failure from the release's overall rollback semantics (e.g. don't make it a blocking hook at all) |
| 18 | `mc` (the MinIO client) has no writable `$HOME` under UDS Core's cluster-wide `runAsNonRoot`/UID-1000 pod mutation, regardless of what the chart's own `securityContext` declares | a bootstrap Job running `mc` loops forever: `mc: <ERROR> Unable to save new mc config. mkdir /.mc: permission denied.` | set `MC_CONFIG_DIR=/tmp/.mc` on the `mc` container's env (`mc`-native override; `/tmp` is writable for arbitrary UIDs under the substrate's enforced posture) — shipped in `sre-apps` gitea rev 1.27.0-4; see `docs/app-onboarding.md` worked example #2 |
| 19 | `srectl app list --installed` reports `drift: live Package without a record` for **every** substrate component that was never installed through the catalog (loki, velero, authservice, falco, prometheus-stack, pgo, vector, grafana, keycloak, and cosmos itself, on a lab where it predates the install-record mechanism) | a long list of "drift" rows on an otherwise fully-healthy cluster, easy to mistake for a problem | **not a bug — semantics, not drift in the alarming sense.** The install-record ConfigMap (`sre-appcatalog-installs`) only tracks catalog-mediated installs (`srectl app install`); every component stood up by the T0/SP2 bring-up procedure in this runbook, or by any pre-catalog deploy, predates and legitimately bypasses that tracking. Expect this noise on every lab that wasn't bootstrapped end-to-end through the catalog; it is not evidence anything is wrong |
| 20 | `zarf package inspect` cannot introspect a **UDS bundle** OCI ref at all, even with the modern, correct subcommand | preflight's advisory inspect step logs `unable to load the package: unknown source ...` for every catalog entry sourced from a bundle (both `cosmos` and `gitea`) | **known, permanent limitation, not a regression.** `zarf package inspect definition <ref>` (the fix for the older "requires a subcommand" bug, PR #32) expects a *native zarf-package* ref, not a *bundle-of-packages* ref — introspecting a bundle needs `uds inspect`/an unwrap step, which `srectl`'s preflight does not call. This means the `no-package-cr` advisory check can never actually run against a bundle-sourced catalog entry, on any zarf/srectl version to date. The separate `missing-require` check (deliberately decoupled from inspect success by the same fix) is unaffected and does run correctly regardless |

Full evidence and root-causing for each: `.superpowers/sdd/gitea-onboarding-plan/task-12a-report.md`,
`task-12a-rerun-report.md` (including its Addendum), `fixA-report.md`, `fixB-report.md`.
